...
class FavouritesManager {
  ...
  init() {
    ...
    document.addEventListener("backend:ready", () => this._loadInitialData(), {
      once: true,
    });
    ...
    if (this._btnFavourite) {
      this._btnFavourite.addEventListener("click", () =>
  ...
  async _loadInitialData() {
    try {
      ...
      this._dispatchStatus(
        "Could not load favourites: " + err.message,
        "warning",
      );
    }
  }
  ...
  _setStarState(isFav, favId) {
    ...
    this._btnFavourite.title = isFav
  ...
  async toggleFavourite() {
    ...
    try {
      if (isFav) {
        ...
        // Add to favourites
        const res = await window.MarkdownViewerAPI.addFavourite(
      ...
      if (msg.includes("does not exist") || msg.includes("not found")) {
        displayMsg =
          "Cannot save favourite: File path not accessible. Try opening the file from the examples folder or using the CLI with a full path.";
      }
  ...
  renderTable() {
    ...
    ["Name", "Filename", "Path", "Tags", "Favourited", "Actions"].forEach(
      (label) => {
  ...
  async saveEdit(favId, name, tags, tr, originalItem) {
    try {
      const res = await window.MarkdownViewerAPI.updateFavourite(favId, {
        name,
        tags,
      });
      if (res.success) {
    ...
    input.addEventListener("keydown", (e) => {
      if (e.key === "Enter" || e.key === ",") {
        ...
        const existing = Array.from(
          wrapper.querySelectorAll(".fav-chip[data-tag]"),
  ...
  _filterSidebar(query) {
    ...
    this._sidebarList.querySelectorAll("li").forEach((li) => {
      ...
      li.style.display =
        !q || name.includes(q) || path.includes(q) ? "" : "none";
    });
  }
